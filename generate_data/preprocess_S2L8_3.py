...
def crop(L8_root,S2_root1,S2_root2,S2_root3,crop_size_h,crop_size_w,prefix,save_dir,crop_label=False):
	...
	raw3_S2_B02 = io.imread(S2_root3+'/B02.tif',dtype=np.uint8)[:,:,0]
	raw3_S2_B03 = io.imread(S2_root3+'/B03.tif',dtype=np.uint8)[:,:,0]
	raw3_S2_B04 = io.imread(S2_root3+'/B04.tif',dtype=np.uint8)[:,:,0]
	raw3_S2_B08 = io.imread(S2_root3+'/B08.tif',dtype=np.uint8)[:,:,0]
	raw3_S2_B11 = io.imread(S2_root3+'/B11_re.tif',dtype=np.uint8)
	raw3_S2_B12 = io.imread(S2_root3+'/B12_re.tif',dtype=np.uint8)

	raw_L8_7 = [raw_L8_B01, raw_L8_B02, raw_L8_B03, raw_L8_B04, raw_L8_B05, raw_L8_B06, raw_L8_B07]
	raw_S2 = [raw1_S2_B02, raw1_S2_B03, raw1_S2_B04, raw1_S2_B08, raw1_S2_B11, raw1_S2_B12, \
				raw2_S2_B02, raw2_S2_B03, raw2_S2_B04, raw2_S2_B08, raw2_S2_B11, raw2_S2_B12, \
				raw3_S2_B02, raw3_S2_B03, raw3_S2_B04, raw3_S2_B08, raw3_S2_B11, raw3_S2_B12]
	...
	while(y2<h_L8 and y2*3<h_S2):
		while(x2<w_L8 and x2*3<w_S2):
			...
			if(x2>w_L8 or y2>h_L8):
				...
				io.imsave(os.path.join(save_dir,'S2_3_vis',prefix+"_%d.tif"%(index)),patch_S2_vis)
				io.imsave(os.path.join(save_dir,'S2_3_label_vis',prefix+"_%d.tif"%(index)),patch_S2_label_vis)

				'''patch_pan_vis = np.uint8((patch_pan-np.min(patch_pan))/(np.max(patch_pan)-np.min(patch_pan))*255)
				io.imsave(os.path.join(save_dir,'pan_vis',prefix+"_%d.tif"%(index)),patch_pan_vis)'''

				'''patch_label_min = np.min(np.reshape(patch_label,(-1,patch_label.shape[2])),axis=0)[0:3]
				patch_label_min = np.expand_dims(np.expand_dims(patch_label_min,axis=0),axis=0)
				patch_label_max = np.max(np.reshape(patch_label,(-1,patch_label.shape[2])),axis=0)[0:3]
				patch_label_max = np.expand_dims(np.expand_dims(patch_label_max,axis=0),axis=0)
				patch_label_vis = np.uint8((patch_label[:,:,0:3]-patch_label_min)/(patch_label_max-patch_label_min)*255)'''
			...
			io.imsave(os.path.join(save_dir,'S2_3',prefix+"_%d.tif"%(index)),patch_S2)
			io.imsave(os.path.join(save_dir,'S2_3_label',prefix+"_%d.tif"%(index)),patch_S2_label)
